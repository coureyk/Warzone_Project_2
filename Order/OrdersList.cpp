...
Order* OrdersList::remove(Node* node) {
    if (isEmpty() || node == NULL) {
        cout << "ERROR. Attempting to remove from empty list or include invalid node.\n\n";
        return NULL;
    }
...
void OrdersList::move(int currentPos, int targetPos) {
    ...
    if (currentNodeFound && targetNodeFound) {
        ...
        cout << "ERROR. Invalid source and target position were provided.\n\n";
    }
    else if (!currentNodeFound) {
        cout << "ERROR. Invalid source position was provided.\n\n";
    }
    else {
        cout << "ERROR. Invalid target position was provided.\n\n";
    }
}
...
//Overloading the operator "<<" so that cout << Order& displays relevant Order information to the user
